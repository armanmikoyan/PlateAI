...
export async function listMealAnalysesForUser(userId: string): Promise<MealAnalysisDocument[]> {
  return MealAnalysis.find({ userId: new Types.ObjectId(userId) })
    .sort({ createdAt: -1 })
    .exec();
}

export async function findMealAnalysisForUser(
  userId: string,
  analysisId: string,
...
export async function updateMealAnalysisForUser(
  userId: string,
  analysisId: string,
...
export async function deletePendingMealAnalysisForUser(
  userId: string,
  analysisId: string,
): Promise<boolean> {
  if (!Types.ObjectId.isValid(analysisId)) {
